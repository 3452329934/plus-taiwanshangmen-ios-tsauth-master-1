...
extension TSImagePickerViewController {
    ...
    class func cameraAnthority() {
        // 1.获取相机授权状态
        let state = AVCaptureDevice.authorizationStatus(forMediaType: AVMediaType.video)
        ...
        if state == .denied {
            ...
            // 3.还没有授权，可以询问一次
            AVCaptureDevice.requestAccess(forMediaType: AVMediaType.video, completionHandler: { (granted) in
