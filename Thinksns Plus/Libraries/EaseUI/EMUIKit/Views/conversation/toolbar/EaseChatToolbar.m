...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:80 type:EMChatToolbarTypeGroup];
    if (self) {
...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:80 type:type];
    if (self) {
...
- (void)_setupSubviews
{
    ...
    [styleChangeButton setImage:[UIImage imageNamed:@"ico_chat_keyboard_voice"] forState:UIControlStateNormal];
    [styleChangeButton setImage:[UIImage imageNamed:@"ico_chat_keyboard"] forState:UIControlStateSelected];
    ...
    [self.recordButton setBackgroundImage:[UIImage colorImageWithColor:[UIColor whiteColor]] forState:UIControlStateNormal];
    [self.recordButton setBackgroundImage:[UIImage colorImageWithColor:[UIColor groupTableViewBackgroundColor]] forState:UIControlStateHighlighted];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"ico_kb_expression"] forState:UIControlStateNormal];
    [self.faceButton setImage:[UIImage imageNamed:@"ico_chat_keyboard"] forState:UIControlStateSelected];
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"ico_chat_keyboard_add"] forState:UIControlStateNormal];
    [self.moreButton setImage:[UIImage imageNamed:@"ico_chat_keyboard_add"] forState:UIControlStateHighlighted];
    [self.moreButton setImage:[UIImage imageNamed:@"ico_chat_keyboard"] forState:UIControlStateSelected];
...
{
    if (_recordView == nil) {
        _recordView = [[EaseRecordView alloc] initWithFrame:CGRectMake(90, 130, 150, 150)];
    }
...
//        _faceView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
        TSSystemEmojiSelectorView* faceView = [[TSSystemEmojiSelectorView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 180)];
        faceView.shouldKeepBottomSafeArea = NO;
        faceView.didSelectedEmojiBlock = ^(NSString * _Nonnull emoji) {
...
{
    // 限制最大文字
    [TSAccountRegex checkAndUplodTextViewWithTextView:textView stringCountLimit:singleMsgMaxWordCount];
