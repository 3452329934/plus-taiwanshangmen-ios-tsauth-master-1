...
extension UISegmentedControl {
    ...
    func setiOS12Style() {
        if #available(iOS 13.0, *) {
            let tintColorImage = UIImage(color: TSColor.main.theme)
            let dividerColorImage = UIImage(color: TSColor.main.theme)
            ...
            layer.borderColor = TSColor.main.theme.cgColor
            layer.borderWidth = 1
            setTitleTextAttributes([NSAttributedString.Key.foregroundColor: UIColor.white], for: .selected)
            setTitleTextAttributes([NSAttributedString.Key.foregroundColor: TSColor.main.theme], for: .normal)
        }
    }
}
