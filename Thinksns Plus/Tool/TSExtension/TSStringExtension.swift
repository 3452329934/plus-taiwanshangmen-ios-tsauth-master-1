...
extension NSMutableAttributedString {
    ...
    func differentColorAndSizeString(first : (firstString: NSString, firstColor: UIColor, firstSize: CGFloat), second : (secondString: NSString, secondColor: UIColor, secondSize: CGFloat)) -> NSMutableAttributedString {
        ...
        noteStr.addAttribute(NSAttributedString.Key.font, value: UIFont.systemFont(ofSize: first.firstSize), range: rangeOne)
        noteStr.addAttribute(NSAttributedString.Key.font, value: UIFont.systemFont(ofSize: second.secondSize), range: rangetwo)
...
extension String {
    ...
    static func ts_customImageMarkdownRegex() -> Regex {
        return Regex("@!\\\\{0,2}\\[(.*?)\\\\{0,2}\\]\\((\\d+)\\)")//'\\\\{0,2}'是因为有时候会多出两个转义符
    }
...
extension String {
    ...
    /// 处理Markdown中文字内容
    func ts_getMarkdownContent() -> String {
        let regex = "@!\\\\?\\[(.*?)\\\\?\\]\\((\\d+)\\)"
        let regular = try! NSRegularExpression(pattern: regex, options: [])
        let url = TSAppConfig.share.rootServerAddress + TSURLPathV2.path.rawValue + TSURLPathV2.Download.files.rawValue
        let content =  regular.stringByReplacingMatches(in: self, options: [], range:  NSRange(location: 0, length: self.count), withTemplate: "![$1](\(url)/$2)")
        return content
    }
...
extension String {
    ...
    func ts_convertCustomMarkdownToEditMarkdown() -> (markdown: String, dicArray: [[String: Int]]) {
        ...
        while String.ts_customImageMarkdownRegex().matches(string) {
            ...
            string = string.replacingOccurrences(of: pattern, with: html, options: .regularExpression, range: matchResult.range)
...
extension String {
  ...
  func   ts_attrStringloc (loc: NSInteger, len: NSInteger, color: UIColor, backgroundColor: UIColor?) -> NSMutableAttributedString {
    ...
    attriString.addAttributes( [NSAttributedString.Key.foregroundColor: color], range: NSRange(location: loc, length: len))
    if backgroundColor != nil {
        attriString.addAttributes([NSAttributedString.Key.backgroundColor: backgroundColor as Any], range: NSRange(location: loc, length: len))
    }
    return attriString
    }
