...
class AppDeleguate: UIResponder, UIApplicationDelegate, JPUSHRegisterDelegate, EMChatManagerDelegate, EMChatroomManagerDelegate, EMGroupManagerDelegate, EMCallManagerDelegate, WXApiDelegate,IMLSDKRestoreDelegate {
    ...
    private var currentApplication: UIApplication!
    private var currentLaunchOptions: [UIApplication.LaunchOptionsKey: Any]?

    // MARK: - Application and setup
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        if UserDefaults.standard.value(forKey: TSCurrentUserInfo.isAccessUserPrivacy) == nil {
            let vc = TSPrivacyNoticeVC()
            vc.sureAciton = {
                self.configApp(self.currentApplication, self.currentLaunchOptions)
            }
            let rootNav = TSNavigationController(rootViewController: vc)
            window?.rootViewController = rootNav
            window?.makeKeyAndVisible()
            currentApplication = application
            currentLaunchOptions = launchOptions
        } else {
            configApp(application, launchOptions)
        }
        return true
    }

    func configApp(_ application: UIApplication, _ launchOptions: [UIApplication.LaunchOptionsKey: Any]?) {
        /// 更新设置中的版本信息
        updateSettingInfo()
        /// 该类初始化之后,配置整个应用主题色等
        ...
        // 注册状态栏改变的通知
//        NotificationCenter.default.addObserver(self, selector: #selector(statusBarFrameDidChange(notice:)), name: UIApplication.didChangeStatusBarFrameNotification, object: nil)
//        NotificationCenter.default.addObserver(self, selector: #selector(statusBarFrameWillChange(notice:)), name: UIApplication.willChangeStatusBarFrameNotification, object: nil)
        /// 应用启动2秒后
...
//        EMClient.shared().applicationDidEnterBackground(application)
    ...
    @objc func statusBarFrameDidChange(notice: Notification) {
    }

    @objc func statusBarFrameWillChange(notice: Notification) {
    ...
    /// 更新设置中的版本信息
    private func updateSettingInfo() {
        if let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String, let build = Bundle.main.infoDictionary?["CFBundleVersion"] as? String {
            let showVersion = version + " " + "build:" + build
            UserDefaults.standard.set(showVersion, forKey: "version_id")
     ...
     */
     ...
     // Called when the application is about to terminate. Save data if appropriate. See also applicationDidEnterBackground:.
        VideoRecorderExportHelper.saveNeedRecoverVideoName()
     }
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        let vc = TSRootViewController.share.tabbarVC
        vc?.changeStatuBar()
    }
}
