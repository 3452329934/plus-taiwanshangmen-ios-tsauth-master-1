...
extension AppDeleguate {
    ...
    func setupDataBaseVersion() {
        ...
        */
                ...
                if oldSchemaVersion < 8 {
                    migration.enumerateObjects(ofType: TSImageObject.className(), { (oldObject, newObject) in
                        newObject!["netImageUrl"] = ""
                    })
                }
                if oldSchemaVersion < 9 {
                    migration.enumerateObjects(ofType: TSImageObject.className(), { (oldObject, newObject) in
                        newObject!["vendor"] = "local"
                    })
                }
        })
    ...
    func setupImageCache() {
        ...
        VideoRecorderExportHelper.recoverVideoToPhotosAlbum()
    }

    // MARK: - 配置环信环境(推送等)
    func setupHY(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) {
        // AppKey: 注册的 AppKey
