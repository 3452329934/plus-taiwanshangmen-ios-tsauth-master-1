...
class TSQuestionDraftObject: Object {
    /// 自增id
    @objc dynamic var draftId: Int = 0
    ...
    /// 正文，纯文字版，用于列表展示
    @objc dynamic var content_text: String?
    /// 是否匿名
    ...
    /// 是否开启悬赏邀请
    @objc dynamic var isOpenOfferRewardInvitation: Bool = false
    /// 邀请的专家/邀请的用户
    @objc dynamic var invitationExpert: TSUserInfoObject?
    /// 是否开启围观
    @objc dynamic var isOpenOutlook: Bool = false
    /// 修改的问题id
    var updatedQuestionId = RealmOptional<Int>()
    /// 是否采纳有答案 - 用于修改时判断能否进入悬赏设置页
    @objc dynamic var isAdoptedAnswer: Bool = false
    /// 创建时间
