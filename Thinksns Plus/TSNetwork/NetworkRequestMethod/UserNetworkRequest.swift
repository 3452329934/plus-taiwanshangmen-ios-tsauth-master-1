...
struct UserNetworkRequest {
    ...
    let receiveComment = Request<TSReceiveSourceModel>(method: .get, path: "user/notifications", replacers: [])
    /// 用户收到的喜欢
    ///
    /// - RouteParameter: None
    /// - RequestParameter:
    ///    - limit: 整数.获取的条数，默认 20。
    ///    - after: 整数.传递上次获取的最后一条 id
    let receiveLike = Request<TSReceiveSourceModel>(method: .get, path: "user/notifications", replacers: [])
    /// 收到的at
    ...
    // 用户协议
    let getUserAgreement = Request<Empty>(method: .get, path: "show-term?type=user_agreement", replacers: [])
    // 隐私政策
    let getPrivacyAgreement = Request<Empty>(method: .get, path: "show-term?type=privacy_agreement", replacers: [])
}
