    ...
    func createTopic(faceNode: String?, topicTitle: String, topicIntro: String?, complete: @escaping ((_ topicId: Int?, _ msg: String?, _ status: Bool, _ needReview: Bool) -> Void)) -> Void {
        ...
        try! RequestNetworkData.share.textRequest(method: requestMethod.method, path: fullPath, parameter: parameter, complete: { (networkResponse, result) in
            ...
            if let needView = resultDict["need_review"] {
                // 请求成功处理
                complete(topicOfId, (needView as! Bool) ? "创建成功，待审核" : "创建成功", true, needView as! Bool)
            } else {
                complete(topicOfId, "创建成功", true, false)
            }
        })
    }
...
extension TSUserNetworkingManager {
    ...
    // 用户协议
    class func getUserAgreement(complete: @escaping (String?, String?, Bool) -> Void) {
       var request = UserNetworkRequest().getUserAgreement
       request.urlPath = request.fullPathWith(replacers: [""])
       request.parameter = ["json": true]
       RequestNetworkData.share.text(request: request) { (networkResult) in
           switch networkResult {
           case .error(_):
                complete(nil, "网络请求错误", false)
           case .failure(let failure):
                complete(nil, failure.message, false)
           case .success(let data):
                if let dic = data.sourceData as? Dictionary<String, Any>, let rules = dic["content"] as? String {
                    complete(rules, data.message, true)
                } else {
                    complete(nil, data.message, true)
                }
           }
       }
    }
    // 隐私政策
    class func getPrivacyAgreement(complete: @escaping (String?, String?, Bool) -> Void) {
       var request = UserNetworkRequest().getPrivacyAgreement
       request.urlPath = request.fullPathWith(replacers: [""])
       request.parameter = ["json": true]
       RequestNetworkData.share.text(request: request) { (networkResult) in
           switch networkResult {
