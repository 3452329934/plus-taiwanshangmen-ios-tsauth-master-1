...
class BindingNetworkManager: NSObject {
    ...
    ///       - true:   走登录
    ///       - false:  表示走三方注册/绑定流程
    func isSocialited(userId: String? = nil, provider: ProviderType, token: String, complete: @escaping ((_ token: TSAccountTokenModel?, _ userModel: TSCurrentUserInfoModel?, _ msg: String?, _ status: Bool) -> Void)) {
        ...
        try! RequestNetworkData.share.textRequest(method: requestMethod.method, path: requestMethod.fullPathWith(replace: provider.rawValue), parameter: parameter, complete: { (response, status) in
            var message: String?
            guard status else {
                message = TSCommonNetworkManager.getNetworkErrorMessage(with: response)
                complete(nil, nil, message, false)
