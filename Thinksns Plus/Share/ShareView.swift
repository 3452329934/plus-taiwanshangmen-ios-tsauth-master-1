...
class ShareView: UIView, Sharable {
    ...
    @objc internal func buttonTaped(_ sender: UIGestureRecognizer) {
        ...
        shareByTypeName(typeName: shareName, finishBlock: finishBlock)
    }
    
    func shareByTypeName(typeName: String, finishBlock: @escaping (Bool) -> Void) {
        switch typeName {
        case "QQ":
    ...
    public func config(URLString: String?, image: UIImage?, description: String?, title: String?) {
