...
//    private let maxWidth: CGFloat
    ...
    private func initialUI() -> Void {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(textViewBeginEditingNotificationProcess(_:)), name: UITextView.textDidBeginEditingNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(textViewEndEditingNotificationProcess(_:)), name: UITextView.textDidEndEditingNotification, object: nil)
...
//        self.delegate?.newInputView(self, didHeightChanged: height)
    ...
    fileprivate func setupTextCount(currentNum: Int) -> Void {
        ...
        attText.addAttribute(NSAttributedString.Key.foregroundColor, value: TSColor.normal.disabled, range: attText.rangeOfAll())
        attText.addAttribute(NSAttributedString.Key.foregroundColor, value: TSColor.normal.disabled, range: NSRange(location: 0, length: currentCount.count))
