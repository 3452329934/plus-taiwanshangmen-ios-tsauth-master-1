...
class ReceiveCommentTableVC: TSTableViewController, NoticePendingProtocol, TSKeyboardToolbarDelegate {
    ...
    override func refresh() {
        ...
        request.urlPath = request.urlPath + "?type=comment"
        RequestNetworkData.share.text(request: request) { (_) in
            // 直接清理本地的数据
            TSCurrentUserInfo.share.unreadCount.comments = 0
        }
        page = 1
        NoticeReceiveInfoNetworkManager.receiveCommentList(after: page) { [weak self] models, originModels, _ in
            ...
            weakSelf.tableView.mj_footer.isHidden = originModels.count < TSAppConfig.share.localInfo.limit
    ...
    override func loadMore() {
        ...
        NoticeReceiveInfoNetworkManager.receiveCommentList(after: page) { [weak self] models, originModels, _ in
            ...
            weakSelf.sourceData += models
            if originModels.count < TSAppConfig.share.localInfo.limit {
                weakSelf.tableView.mj_footer.endRefreshingWithNoMoreData()
            }
            weakSelf.tableView.reloadData()
        }
