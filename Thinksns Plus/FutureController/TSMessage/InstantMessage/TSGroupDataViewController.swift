...
class TSGroupDataViewController: TSViewController, TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func creatSubView() {
        ...
        groupImageView.contentMode = UIView.ContentMode.scaleAspectFill
        ...
        groupManagerIconHeader.contentMode = UIView.ContentMode.scaleAspectFill
        ...
        changeImageButton.addTarget(self, action: #selector(changeGroupImage), for: UIControl.Event.touchUpInside)
        ...
        groupManagerIconName.contentMode = UIView.ContentMode.scaleAspectFill
        if isOwner == false {
        ...
        changeNameButton.addTarget(self, action: #selector(changeGroupName), for: UIControl.Event.touchUpInside)
        ...
        groupManagerIcon.contentMode = UIView.ContentMode.scaleAspectFill
        ...
        groupManagerButton.addTarget(self, action: #selector(enterGroupManager), for: UIControl.Event.touchUpInside)
        ...
        switchView.addTarget(self, action: #selector(blockMessage), for: UIControl.Event.valueChanged)
        ...
        cleanMessageButton.addTarget(self, action: #selector(cleanMessageButtonClick), for: UIControl.Event.touchUpInside)
        ...
        leaveGroupButton.addTarget(self, action: #selector(leaveGroupButtonClick), for: UIControl.Event.touchUpInside)
    ...
    func setMembersUI() {
        ...
        for (index, item) in groupMemberShowArray.enumerated() {
            ...
            if index == groupMemberShowArray.count - 1 {
                ...
                addButton.addTarget(self, action: #selector(addMembersButtonClick), for: UIControl.Event.touchUpInside)
                if isOwner {
                    /// 删除成员按钮
                    ...
                    deleteBuuton.addTarget(self, action: #selector(deleteMemberButtonClick), for: UIControl.Event.touchUpInside)
                }
            }
        }
        // 人数够了才显示查看更多,否则不显示
        if (isOwner == true && groupMembers.count > 18) || (isOwner == false && groupMembers.count > 19) {
            ...
            lookMoreMemberButton.addTarget(self, action: #selector(lookMoreButtonClick), for: UIControl.Event.touchUpInside)
        }
    }
    ...
    // MARK: - 点击了用户头像
    @objc func userItemDidTap(rec: UITapGestureRecognizer) {
...
//            imagePickerVC.selectImage = UIImage(named: "msg_box_choose_now")
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let type = info[UIImagePickerController.InfoKey.mediaType] as? String, type == "public.image", let photo = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
            let photoOrigin = photo.fixOrientation()
            ...
            lzImage.modalPresentationStyle = .fullScreen
    ...
    @objc func changeGroupImageRequest(imageId: Int) {
    ...
    // MARK: - 清空聊天记录
    @objc func cleanMessageButtonClick() {
    ...
    // MARK: - 屏蔽消息
    @objc func blockMessage(currentSwitchView: UISwitch) {
        let groupid = "\(self.originData["id"] ?? "")"
    ...
    // MARK: - 退群或者解散群
    @objc func leaveGroupButtonClick() {
        let groupid = "\(self.originData["id"] ?? "")"
        if isOwner {
            /// 解散群
    ...
    // MARK: - 添加成员
    @objc func addMembersButtonClick() {
    ...
    // MARK: - 删除成员
    @objc func deleteMemberButtonClick() {
        let vc = TSChatFriendListViewController()
