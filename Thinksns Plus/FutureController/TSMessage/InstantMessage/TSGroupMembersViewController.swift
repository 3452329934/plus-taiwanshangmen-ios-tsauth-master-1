...
class TSGroupMembersViewController: TSViewController {
    ...
    func setMembersUI() {
        ...
        addButton.addTarget(self, action: #selector(addMember), for: UIControl.Event.touchUpInside)
        ...
        deleteButton.addTarget(self, action: #selector(deleteMember), for: UIControl.Event.touchUpInside)
...
//            let otherButtonX = space + otherWidth + CGFloat(index) * gSpace
    ...
    // MARK: - 刷新群成员的数据再布局ui
    @objc func reloadGroupMemberInfo(notice: Notification) {
    ...
    // MARK: - 进去个人中心
    @objc func interUsersHomePage(btn: Button) {
