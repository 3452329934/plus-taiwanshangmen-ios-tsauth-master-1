...
class ChatListViewController: TSViewController, UISearchBarDelegate {
    ...
    /// IM第二版聊天列表页面
    var chatListVC: TSChatListTableViewController!
    ...
    override func viewDidLoad() {
        ...
        setSearchBarUI()
        self.chatListVC = TSChatListTableViewController(style: .plain, model: ChatListViewController.pasteNiticeModel())
        self.chatListVC.superViewController = self.superViewController
        self.chatListVC.superMessagePop = self.superMessagePop
        self.chatListVC.pViewController = self
    ...
    static func pasteNiticeModel() -> [NoticeConversationCellModel] {
        if TSCurrentUserInfo.share.isLogin == false {
            ...
            let systemModel = NoticeConversationCellModel(title: "系统消息", content: unreadInfo.commentsUsers ?? "暂无系统消息".localized, badgeCount: unreadInfo.comments, date: unreadInfo.commentsUsersDate, image: "ico_message_systerm")
            let commentModel = NoticeConversationCellModel(title: "收到的评论".localized, content: unreadInfo.commentsUsers ?? "显示_收到的评论占位字".localized.localized, badgeCount: unreadInfo.comments, date: unreadInfo.commentsUsersDate, image: "IMG_message_comment")
