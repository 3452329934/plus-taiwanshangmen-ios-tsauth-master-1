...
struct NoticePendingCellLayoutConfig {
    /// 待操作区的状态
    var pendingReginStatus: NoticePendingCellPendingReginStatus
    /// 是否隐藏扩展区域
    var isHiddenExtenRegin: Bool
    /// 是否隐藏正文
    var isHiddenContent: Bool
    /// 头像
    var avatarUrl: String?
    /// 头像认证类型
    var verifyType: String?
    /// 头像认证标识
    var verifyIcon: String?
    /// 头像用户id
    var userId: Int?
    /// 标题
    var title: String
    ...
    override func layoutSubviews() {
        ...
        if self.isHidden == false {
            ...
            content.snp.remakeConstraints { (mark) in
                mark.top.equalToSuperview().offset(-6)
                mark.right.equalToSuperview().offset(-5)
                mark.bottom.equalToSuperview().offset(6)
                if cover.isHidden {
                    mark.left.equalToSuperview().offset(5)
                } else {
    ...
    override func loadConfig() {
        ...
        if config.isHiddenExtenRegin == false { // 是否显示封面
            ...
            if let extenContent = config.extenContent {
                extenView.content.attributedText = extenContent.attributonString().setTextFont(12).setlineSpacing(6)
