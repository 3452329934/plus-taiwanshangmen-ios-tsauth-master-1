...
class TSGroupManagerVC: TSViewController {
    ...
    func creatSubViews() {
        ...
        groupManagerButton.addTarget(self, action: #selector(changeGroupOwner), for: UIControl.Event.touchUpInside)
    ...
    // MARK: - 开启进群审核
    @objc func blockMessage(switchView: UISwitch) {

    }

    // MARK: - 转让群主
    @objc func changeGroupOwner() {
        ...
        vc.dismissBlock = {
            /// 返回到聊天详情页面
            if let childVCList = self.navigationController?.children {
