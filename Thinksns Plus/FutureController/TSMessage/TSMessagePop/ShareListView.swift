...
//        }
    ...
    func shareByShareName(_ shareName: String, fatherViewTag index: Int, finishBlock: @escaping (Bool) -> Void) {
        switch shareName {
        case "QQ":
    ...
    @objc internal func buttonTaped(_ sender: UIGestureRecognizer) {
        ...
        shareByShareName(shareName, fatherViewTag: index, finishBlock: finishBlock)
    }
    ...
    public func show(URLString: String?, image: UIImage?, description: String?, title: String?) {
        config(URLString: URLString, image: image, description: description, title: title)
        show()
    }
    
    func config(URLString: String?, image: UIImage?, description: String?, title: String?) {
   ...
   @objc public func dismiss() {
    ...
    func getVideoShareItems() -> [ShareItemModel] {
        var items = [ShareItemModel]()
        for index in 0..<imageArray.count {
            let item = ShareItemModel(title: titleArary[index], icon: UIImage(named: imageArray[index]), tag: titleArary[index])
            items.append(item)
        }
        return items
    }
