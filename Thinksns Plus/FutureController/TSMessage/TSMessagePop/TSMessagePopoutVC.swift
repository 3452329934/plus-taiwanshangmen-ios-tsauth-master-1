...
//        whiteView.addSubview(firstLine)
        ...
        cancelButton.addTarget(self, action: #selector(hidSelf), for: UIControl.Event.touchUpInside)
...
extension TSMessagePopoutVC {
    ...
    public func show(vc: TSMessagePopoutVC) {
        ...
        window.rootViewController?.didMove(toParent: window.rootViewController)
    ...
    func setInfo(model: TSmessagePopModel) {
        ...
        let atr = [NSAttributedString.Key.foregroundColor: TSColor.main.theme]
    ...
    @objc func textViewDidChanged(notification: Notification) -> Void {
        // textView判断
