...
class TSSetUserInfoVC: TSViewController, UITextViewDelegate, UITextFieldDelegate, TSCustomAcionSheetDelegate, UIScrollViewDelegate, TSUserInfoLabelDelegate, TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // 增加检测键盘输入状态的通知
        NotificationCenter.default.addObserver(self, selector: #selector(self.textFiledDidChanged(notification:)), name: UITextField.textDidChangeNotification, object: nil)
...
//            imagePickerVC.selectImage = UIImage(named: "msg_box_choose_now")
    ...
    private func changeImageRequest(image: UIImage, imageName: String, size: CGSize) {
        ...
        let imageData = cropped.jpegData(compressionQuality: 1.0)!
        // 修改用户头像
    ...
    private func setCountLabel(intro: String) {
        if intro.count > textFiledMaximumWordLimit {
            personalProfileWithCountLabelOfTop.priority = UILayoutPriority.defaultHigh
            personalpProfileWithSuperViewOfBottom.priority = UILayoutPriority.defaultLow
            currentTextNumberLabel.isHidden = false
            currentTextNumberLabel.text = "\(intro.count)/\(textViewMaximumWordLimit)"
        } else {
            personalProfileWithCountLabelOfTop.priority = UILayoutPriority.defaultLow
            personalpProfileWithSuperViewOfBottom.priority = UILayoutPriority.defaultHigh
    ...
    @objc func toSettingUserLabelVC() {
        // 进入标签选择界面
