...
class TSEnterpriseBasicInfoVC: TSTableViewController, UITextViewDelegate {
    ...
    /// 负责人
    @IBOutlet weak var textfildForManager: UITextField!
    /// 身份证号码
    ...
    /// 输入框输入状态记录
    /// - 有值为 true, 反之为 false
    var accountUsable = ["name": false, "adress": false, "manager": false, "idcard": false, "phone": false, "description": false]
    /// 认证显示 labelForWordsCount 的字数
    let showWordsCountLabelLimit = 170
    /// 认证描述最大字数
    let maxWordsCountForDescription = 200
    /// 机构名字最大字数
    let maxWordsCountForName = 20
    /// 机构地址最大字数
    let maxWordsCountForAdress = 50
    /// 负责人名称最大字数
    let maxWordsCountForManager = 8
    /// 电话的最大字数
    ...
    // MARK: - Lifecycle
    class func basicInfoVC() -> TSEnterpriseBasicInfoVC {
        let sb = UIStoryboard(name: "TSEnterpriseCertification", bundle: nil)
        let vc = sb.instantiateViewController(withIdentifier: "TSEnterpriseBasicInfoVC") as! TSEnterpriseBasicInfoVC
        return vc
    }
    ...
    func setUI() {
        // 增加检测键盘输入状态的通知
        NotificationCenter.default.addObserver(self, selector: #selector(textFiledDidChanged(notification:)), name: UITextField.textDidChangeNotification, object: nil)
    ...
    @objc func textFiledDidChanged(notification: Notification) {
        // 输入框输入文字上限
        ...
        if let textField = notification.object as? UITextField {
            ...
            switch textField {
            case textfieldForName:
                stringType = "name"
                stringCountLimit = maxWordsCountForName
            case textfieldForAdress:
                stringType = "adress"
                stringCountLimit = maxWordsCountForAdress
            case textfildForManager:
                stringType = "manager"
                stringCountLimit = maxWordsCountForManager
    ...
    @IBAction func sureButtonTaped() {
        ...
        // 1.用 model 保存用户输入的信息
        model.orgName = textfieldForName.text!
        model.orgAddress = textfieldForAdress.text!
        model.name = textfildForManager.text!
        ...
        vc.finishOperation = { [weak self] (imageIds: [Int], imageSize: [CGSize], alert: TSIndicatorWindowTop?)  in
            ...
            TSDataQueueManager.share.userInfoQueue.uploadCertificate(object: weakSelf.model, complete: { (isSuccess, message) in
                alert?.dismiss()
                guard let weakSelf = self else {
                    return
                }
                ...
                // 跳转回一开始的界面
                guard var count = weakSelf.navigationController?.children.count, isSuccess else {
                    return
                }
                count -= 3
                guard let backVC = weakSelf.navigationController?.children[count] else {
                    return
                }
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        labelForWordsCount.sizeToFit()
        // 5.根据高度判断 textView 是否可滚动
        if textView.isEqual(textviewForDescription) && textviewForDescription.contentSize.height >= 90 {
