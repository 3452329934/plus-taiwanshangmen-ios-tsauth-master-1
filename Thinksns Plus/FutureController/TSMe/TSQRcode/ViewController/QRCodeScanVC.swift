...
//        tishiLabel.text = "将二维码放入条框内，即可自动扫码"
    ...
    lazy var openButton: UIButton = {[unowned self] in
        ...
        openButton.addTarget(self, action: #selector(openFlashlight), for: UIControl.Event.touchUpInside)
    ...
    lazy var qrImageView: UIImageView = {[unowned self] in
        let qrImageView = UIImageView(frame: CGRect(x: 0, y: -qrWidth, width: qrWidth, height: qrWidth))
        qrImageView.image = UIImage(named: "pic_saoma")
        return qrImageView
        }()
...
//        qrImageView.startAnimating()
    ...
    @objc func animation() {
        qrImageView.mj_y += 1
        if qrImageView.frame.origin.y >= 0 {
            qrImageView.mj_y = -qrWidth
        }
    }
...
//    }
    ...
    func createSession() {
        ...
        do {
            ...
            view.bringSubviewToFront( tishiLabel)
...
//        qrImageView.stopAnimating()
    ...
    @objc func openFlashlight(_ sender: UIButton) {
