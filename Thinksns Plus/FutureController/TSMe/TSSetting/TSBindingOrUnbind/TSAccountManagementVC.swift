...
class TSAccountManagementVC: TSViewController, TSBindingOrUnbindTableViewDelegate, TSCustomAcionSheetDelegate {
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 13.0, *) {
            arry.append(TSBindingItme(nameType: .apple, status: false))
        }
        setUI()
        ...
        TSUserNetworkingManager().getCurrentUserInfo { (userModel, _, status) in
            ...
            BindingNetworkManager().getUserSocialite { [weak self] (result, status) in
                ...
                for str in result {
                    ...
                    if str == ProviderType.apple.rawValue {
                        for (index, model) in self.arry.enumerated() {
                            if model.nameType == .apple {
                                self.arry[index].status = true
                            }
                        }
                    }
                }
    ...
    func itemForDataSoruce(item: TSBindingItme) {
        ...
        case .apple:
            if self.arry[0].status == true {
                bindingOrUnbindApple(status: item.status)
            } else {
                let faild = TSIndicatorWindowTop(state: .faild, title: "绑定三方账号前,需要绑定手机号")
                faild.show(timeInterval: 2)
            }
    ...
    ///     - true:     已绑定：走解绑
    ///     - false:    未绑定：走绑定
    func bindingOrUnbindApple(status: Bool) {
       guard status == false else {
           unbindForUserSocialite(type: .apple)
           return
       }
        ...
        TSSignInAppleManager.share().didGetAuthInfoAction = { [unowned self] (uid, token) in
            HUD.show(HUDContentType.progress)
            var request = BindingNetworkRequest().loginUserProvider
            request.urlPath = request.fullPathWith(replacers: ["apple"])
            let parameter: [String: Any] = ["userId": uid,"access_token": token]
            request.parameter = parameter
