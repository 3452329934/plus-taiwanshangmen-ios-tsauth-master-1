...
class TSHomeTabBarController: UITabBarController, HomeTabBarCenterButtonDelegate, UITabBarControllerDelegate, TSBeforeReleaseViewDelegate, DemoCallManagerDelegate {
    ...
    /// 当编辑视频时是否暂停了音乐播放
    var isPausedMusic = false
    /// 记录是否是第一次开启热点
    var isFirstOpenHot = true
...
//            NotificationCenter.default.addObserver(self, selector: #selector(changeStatuBar), name: NSNotification.Name.UIApplication.didChangeStatusBarFrameNotification, object: nil)
        }
    ...
    @objc func changeStatuBar() {
        ...
        var shouldBeSubtractionHeight: CGFloat = 0
        if rect.size.height == TSStatusBarHeight * 2 {
            shouldBeSubtractionHeight = TSStatusBarHeight * 2
        }
        let statusBarHeight = ScreenHeight - 49 - shouldBeSubtractionHeight
        customTabBar.frame = CGRect(x: 0, y:statusBarHeight, width: ScreenWidth, height: 49)
    }
...
//        self.addChild(TSMomentListVC(), "标题_首页".localized, "IMG_common_ico_bottom_home_normal", "IMG_common_ico_bottom_home_high")
        ...
        addChild(messageVC, "标题_消息".localized, "IMG_common_ico_bottom_message_normal", "IMG_common_ico_bottom_message_high")

        self.addChild(TSMeViewController(), "标题_我".localized, "IMG_common_ico_bottom_me_normal", "IMG_common_ico_bottom_me_high")
    ...
    internal func tabbarCenterButtonTap(_ tabbar: TSHomeTabBar) {
        NotificationCenter.default.post(name: NSNotification.Name("FeedListViewReloadData"), object: nil, userInfo: nil)
...
//            imagePickerVC.selectImage = UIImage(named: "msg_box_choose_now")
    ...
    /// 收到APNs 切换视图
    @objc func changeView(_ noti: NSNotification) {
        // 关闭所有的视图
    ...
    @objc func receiveNotice() {
        customTabBar.showBadge(.message)
    }
    // MARK: - 全局音乐悬浮窗口点击事件处理
    @objc func didClickMusicWindow() {
        guard let nav = self.children[selectedIndex] as? TSNavigationController else {
            return
        }
...
extension TSHomeTabBarController: TZImagePickerControllerDelegate {
    func imagePickerControllerDidClickTakePhotoBtn(_ picker: TZImagePickerController!) {
        // 进入视频录制
        // 视频录制完毕后 进入发布页面
        isPausedMusic = TSMusicPlayVC.shareMusicPlayVC.needPaused()
    ...
    func imagePickerController(_ picker: TZImagePickerController!, didFinishEditVideoCover coverImage: UIImage!, videoURL: Any!) {
        ...
        present(nav, animated: true)
        if isPausedMusic {
            TSMusicPlayVC.shareMusicPlayVC.needPlay()
        }
    }
...
extension TSHomeTabBarController {
    ...
    func addChild(_ childController: UIViewController, _ title: String, _ normalImageName: String, _ selectedImageName: String) {
...
extension TSHomeTabBarController: UINavigationControllerDelegate {
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        if viewController is ZLEditVideoController {
            isPausedMusic = TSMusicPlayVC.shareMusicPlayVC.needPaused()
        }
    }
}
