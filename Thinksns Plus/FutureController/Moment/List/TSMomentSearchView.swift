...
protocol TSMomentSearchViewDelegate: class {
    func didClickVideoCell(_ feedListView: TSMomentSearchView, cellIndexPath: IndexPath, fatherViewTag: Int)
    /// 当tableView 滑动停止后如果屏幕上出现了可以播放视频的cell
    func canPlayVideoCell(_ feedListView: TSMomentSearchView, indexPath: IndexPath)
}

class TSMomentSearchView: FeedListView {
    ...
    weak var feedListViewDelegate: TSMomentSearchViewDelegate?
...
extension TSMomentSearchView: FeedListViewDelegate {
    /// 点击了动态 cell
...
extension TSMomentSearchView: ShareListViewDelegate {
...
extension TSMomentSearchView: TSKeyboardToolbarDelegate {
...
extension TSMomentSearchView: FeedListViewRefreshDelegate {
    // MARK: 代理方法
    /// 下拉刷新
    func feedListTable(_ table: FeedListView, refreshingDataOf tableIdentifier: String) {
        // 2.发起网络请求
        FeedListNetworkManager.getSearchFeeds(keyword: keyword, type: "new", after: nil) { [weak self] (model: FeedListResultsModel?, message: String?, status: Bool) in
            // 3.根据列表类型，对数据进行不同的处理
    ...
    func feedListTable(_ table: FeedListView, loadMoreDataOf tableIdentifier: String) {
        ...
        var id: Int? = nil
        id = table.after
        FeedListNetworkManager.getSearchFeeds(keyword: keyword, type: "new", after: id) { (model: FeedListResultsModel?, message: String?, status: Bool) in
            // 2.处理数据
            var feedModels: [FeedListCellModel]?
            if let model = model {
    ...
    func processReloadNewDatas(_ originalModel: FeedListResultsModel?, complete: @escaping ([FeedListCellModel]?, Int) -> Void) {
        ...
        pinnedModels = pinnedModels.filter { (model) -> Bool in
            if (model.content.contains(keyword)) {
                return true
            }
            return false
        }
