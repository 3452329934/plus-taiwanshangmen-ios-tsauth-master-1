...
class FeedPagesController: TSLabelViewController, ZFPlayerDelegate {
    ...
    /// 当前显示的视图
    var currentWaitPlayView: FeedListActionView?
    /// 当前正在播放视频的视图
    var currentWaitPlayCell: FeedListCell?
    /// 播放器控制视图
    var playerControl: CustomPlayerControlView?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if self.playerView != nil, didAddObserToPlayerViewPlayState == false {
            playerView?.addObserver(self, forKeyPath: "isPauseByUser", options: [.new, .old], context: nil)
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if self.playerView != nil, didAddObserToPlayerViewPlayState == true {
            playerView?.removeObserver(self, forKeyPath: "isPauseByUser")
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        let playState = !self.playerView.isPauseByUser
        print("视频播放状态b改变 和 全局定义的 = \(playState) = \(shortVideoState)")
        if self.playerView != nil && TSCurrentUserInfo.share.isAgreeUserCelluarWatchShortVideo == false && TSAppConfig.share.reachabilityStatus == .Cellular {
            if playState == true {
                ...
                UIApplication.shared.keyWindow?.rootViewController?.view.addSubview(alert.view)
                UIApplication.shared.keyWindow?.rootViewController?.addChild(alert)
                alert.didMove(toParent: UIApplication.shared.keyWindow?.rootViewController)
            }
        }
    ...
    @objc func appWillResignActive() {
        ZFBrightnessView.shared().isLockScreen = true
...
//            UIApplication.shared.keyWindow?.rootViewController?.present(alert, animated: false, completion: nil)
    ...
    @objc func didClickShortVideoShareBtn(_ sender: Notification) {
        configShareView()?.show()
    }
    
    func configShareView() -> ShareView? {
        // 当分享内容为空时，显示默认内容
    ...
    @objc func didClickAdvertToolBtn(_ sender: Notification) {
    ...
    /// 添加话题下面发布的新动态
    @objc func addNewTopicFeed(_ notification: Notification) {
        // 2.解析通知发送的信息
    ...
    func playVideoWith(_ feedListView: FeedListActionView, indexPath: IndexPath) {
        ...
        if let cell = self.currentPlayingCell {
            if let image = cell.picturesView.pictureViews.first?.picture {
        ...
        if playerControl == nil, let cell = self.currentPlayingCell {
            playerControl = CustomPlayerControlView()
            playerControl!.showFullScreenPlayEndView = true
            playerControl!.fullScreenPlayEndView.imageColor = UIColor.white
            playerControl!.zf_playerPlayEndBlock = {
                   cell.playIcon.isHidden = true
            }
        }
        if let shareView = self.configShareView() {
            playerControl!.setFullScreenShareItemsModel(shareView.getVideoShareItems()) { (model) in
                shareView.shareByTypeName(typeName: model.tag) { states in
                    //分享完成
                }
            }
        }
        self.playerView.playerControlView(playerControl, playerModel: playModel)
        /// 由于player的实现不会显示播发按钮，并且应用进入前台后会继续播放(没有设置isPauseByUser)所以删除player的监听，在此类中实现监听
        NotificationCenter.default.removeObserver(self.playerView, name: UIApplication.willResignActiveNotification, object: nil)
        self.playerView.playerLayerGravity = ZFPlayerLayerGravity.resizeAspect
        self.playerView.hasDownload = true
        self.playerView.delegate = self
        self.playerView.autoPlayTheVideo()
        //当存在占位图时隐藏高斯模糊
//        for item in self.playerView.placeholderBlurImageView.subviews {
//            if item is UIVisualEffectView {
//                item.isHidden = self.playerView.placeholderBlurImageView.image == nil ? false : true
//            }
//        }
        if TSMusicPlayerHelper.sharePlayerHelper.player.status == .Playing {
            TSMusicPlayVC.shareMusicPlayVC.playOrStop("")
    ...
    @objc func reachabilityChanged(note: NSNotification) {
        ...
        if (reachability?.isReachable)! {
            if (reachability?.isReachableViaWiFi)! {
                if let waitPlayView = currentWaitPlayView, let waitPlayCell = currentWaitPlayCell, currentShowPage == waitPlayView  {
                    if waitPlayView.visibleCells.contains(waitPlayCell) {
                        if let indexPath = waitPlayView.indexPath(for: waitPlayCell) {
                            self.playVideoWith(waitPlayView, indexPath: indexPath)
                        }
                    }
                }
                //当页面未显示时
                currentWaitPlayView = nil
                currentWaitPlayCell = nil
            }
        }
    }
}
...
extension FeedPagesController: FeedListActionViewDelegate {
    func didClickVideoCell(_ feedListView: FeedListActionView, cellIndexPath: IndexPath, fatherViewTag: Int) {
        ...
        if TSAppConfig.share.reachabilityStatus == .WIFI {
            ...
            alert.addAction(action)
            UIApplication.shared.keyWindow?.rootViewController?.present(alert, animated: false, completion: nil)
            currentWaitPlayView = feedListView
        ...
        FeedListNetworkManager.getTypeFeeds(type: tableIdentifier, after: nil) { [weak self] (model: FeedListResultsModel?, message: String?, status: Bool) in
            ...
            case .hot:
                ...
                self?.processReloadHotDatas(model, complete: { (feedListCellModels, pinnedCounts) in
                    ...
                    (table as? FeedListActionView)?.pinnedCounts = pinnedCounts
