...
extension FeedCommentLabel: TYAttributedLabelDelegate {
    ...
    func attributedLabel(_ attributedLabel: TYAttributedLabel!, textStorageLongPressed textStorage: TYTextStorageProtocol!, on state: UIGestureRecognizer.State, at point: CGPoint) {
        // 避免多次响应
        if state == .began {
    ...
    // 长按了非文字响应区域
    func attributedLabel(_ attributedLabel: TYAttributedLabel!, lableLongPressOn state: UIGestureRecognizer.State, at point: CGPoint) {
        if state == .began {
            feedCommentDelegate?.feedCommentLabelDidLongpress(self)
        }
    }
...
extension NSMutableAttributedString {
    ...
    class func attributeStringWith(strings: [String], colors: [UIColor], fonts: [CGFloat]) -> NSMutableAttributedString {
        ...
        for (index, text) in strings.enumerated() {
            ...
            attributeStr.addAttributes([NSAttributedString.Key.foregroundColor: color, NSAttributedString.Key.font: font], range: NSRange(location: 0, length: (text as NSString).length))
            // 3.将文字内容拼接起来
