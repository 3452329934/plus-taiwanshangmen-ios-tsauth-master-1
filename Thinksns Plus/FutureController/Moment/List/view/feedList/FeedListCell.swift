...
//        contentView.addSubview(resendButton)
    ...
    internal func loadContentLabel(topRecord: inout CGFloat) {
        ...
        if !shouldAddFuzzyString {
            let attribute = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 15), NSAttributedString.Key.paragraphStyle: paragraphStyle.copy(), NSAttributedString.Key.strokeColor: TSColor.main.theme]
    ...
    func setTopicUI(datas: [TopicListModel], originYY: CGFloat) {
        ...
        if !datas.isEmpty {
            for (index, item) in datas.enumerated() {
                ...
                tagLabel.addTarget(self, action: #selector(jumpToTopicDetailVC(sender:)), for: UIControl.Event.touchUpInside)
