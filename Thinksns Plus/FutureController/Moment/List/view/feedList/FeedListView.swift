...
class FeedListView: TSTableView {
    ...
    init(frame: CGRect, tableIdentifier identifier: String) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(FeedListViewReloadData(noti:)), name: NSNotification.Name("FeedListViewReloadData"), object: nil)
    }
    
    @objc func FeedListViewReloadData(noti: Notification) {
        reloadData()
    }
...
extension FeedListView: UITableViewDelegate, UITableViewDataSource {
    ...
    // MARK: - 更新付费内容显示
    @objc func notiResReloadPaiedFeed(noti: Notification) {
        /// 付费当前页的列表刷新也是同一刷新的
    ...
    @objc func momentDetailVCDelete(noti: Notification) {
...
extension FeedListView {
    ...
    func getCurrentGifPicture() {
        ...
        for index in 0..<self.visibleCells.count {
            ...
            if visibleCells.count > index, let cell = visibleCells[index] as? FeedListCell {
                for indexGif in 0 ..< cell.picturesView.models.count {
                    if cell.picturesView.models.count > indexGif, cell.picturesView.models[indexGif].mimeType == "image/gif" {
                        if cell.picturesView.models[indexGif].paidInfo?.type == .pictureSee {
                            /// 需要付费的不处理（没有付费的）
                        } else {
                            /// 这里需要计算这个gif图片是不是满足可见面积达到百分之八十
                            if cell.picturesView.pictureViews.count > indexGif {
                                let item = cell.picturesView.pictureViews[indexGif]
                                let coverViewPoint = item.convert(CGPoint(x: 0, y: 0), to: self.superview)
                                if coverViewPoint.y < 0 {
                                    if (coverViewPoint.y + item.frame.size.height * 0.2) >= 0 {
                                        hasGifAndCanPlay = true
                                        curentCellIndex = index
                                        break
                                    }
                                } else {
                                    if (coverViewPoint.y + item.frame.size.height * 0.8) <= (self.superview?.frame.height)! {
                                        hasGifAndCanPlay = true
                                        curentCellIndex = index
                                        break
                                    }
        ...
        if hasGifAndCanPlay {
            if visibleCells.count > curentCellIndex, let cell = visibleCells[curentCellIndex] as? FeedListCell {
                TSAnimationTool.animationManager.currentTable = cell.picturesView
