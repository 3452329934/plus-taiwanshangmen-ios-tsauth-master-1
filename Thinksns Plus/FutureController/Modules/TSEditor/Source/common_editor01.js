...
webeditor.contentFocus = function() {
    ...
    webeditor.callNative("contentfocus");
}
...
webeditor.contentBlur = function() {
    ...
    webeditor.callNative("appContentBlur");
}
...
webeditor.contentInput = function() {
    ...
    webeditor.callNative("appContentChange");
}
...
webeditor.imageClick = function(imageIndex) {
    ...
    webeditor.callNative("appImageClick", imageIndex);
}

// imageDelete
webeditor.imageDelete = function(imageIndex) {
    webeditor.debug('imageDelete');
    webeditor.callNative("appImageDelete", imageIndex);
}
...
webeditor.setScrollPosition = function() {
    ...
    webeditor.callNative("appScrollPosition", index);
}
...
webeditor.enableEditingTextStyleItems = function(items) {
    if (items.length > 0) {
        ...
        webeditor.callNative("appEnableEditingStyleItems", items.join(','));
    } else {
        webeditor.debug('enableEditingStyleItems: null');
        webeditor.callNative("appEnableEditingStyleItems");
    }
}

webeditor.callNative = function(funcName, params = '') {
...
webeditor.backuprange = function(){
    ...
    webeditor.debug(selection);

    var range = selection.getRangeAt(0);
    webeditor.debug(range);
...
webeditor.getContentText = function() {
    webeditor.callNative('getContentText', $('#webeditor_content').text());
}
...
//    var markdown = webeditor.getHTML().replace(/<div>|<\/div>|<[divimg]+ class=".*?">/g, '').replace(/\n|\t/g,'').trim();
    ...
    markdown = markdown.replace(/^(?:<p.*?>\s*)*/m, '')

    webeditor.callNative('getContentMarkdown', markdown);
}
