...
/// 资讯web编辑器界面
class TSNewsWebEditorController: TSWebEditorBaseVC {
...
extension TSNewsWebEditorController {
    ...
    /// 是否展示 放弃发布提示弹窗
    func couldShowGiveupPublishDialog(complete: @escaping(_ could: Bool)->Void) {
        ...
        alertVC.addAction(TSAlertAction(title: "选择_确定".localized, style: .default, handler: { (action) in
            if self.editedNewsDetail?.verifyState != NewsVerifyState.rejected {
                self.getImageIds { (ids) in
                    if let ids = ids {
                        super.removeImageCaches(fileIds: ids) // 缓存图片移除
                    }
                }
            }
    ...
    override func nextProcess() {
        ...
        getImageIds { (ids) in
            ...
            let complete = {
                ...
                // 发布模型更新
                self.contributeModel?.content_markdown = getMarkdown
...
extension TSNewsWebEditorController {
    ...
    @objc override func leftItemClick() {
        ...
        // 注：资讯没有草稿箱
        couldShowGiveupPublishDialog { (could) in
            if could {
