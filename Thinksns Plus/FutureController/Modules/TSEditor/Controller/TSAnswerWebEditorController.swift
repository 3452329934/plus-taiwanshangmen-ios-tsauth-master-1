...
/// 答案web编辑器
class TSAnswerWebEditorController: TSWebEditorBaseVC {
        ...
        let complete = {
            ...
            switch self.editType {
                case .normal:
                   ...
                   self.publishAnswer(questionId: questionId, markdown: getMarkdown, content: getContent)
                case .update:
                   // 编辑答案的请求
                   guard let answer = self.editedAnswer else {
                       return
                   }
                   self.updateAnswer(answerId: answer.id, markdown: getMarkdown, content: getContent)
                case .draft:
                   guard let draftModel = self.editedDraft else {
                       return
                   }
                   if let answerId = draftModel.answerId {
                       self.updateAnswer(answerId: answerId, markdown: getMarkdown, content: getContent)
                   } else {
                       self.publishAnswer(questionId: draftModel.questionId, markdown: getMarkdown, content: getContent)
                   }
            }
        }

        didGetMarkdownAction = {[unowned self] (data) in
            markdown = data
            self.didGetMarkdownAction = nil
            didGetMarkdown = true
            complete()
        }
        didGetContentAction = {[unowned self] (data) in
            content = data
    ...
    override func saveDraft() {
        ...
        let complete = {
            ...
            case .normal:
               ...
               draftModel.markdown = getMarkdown
               draftModel.content = getContent
