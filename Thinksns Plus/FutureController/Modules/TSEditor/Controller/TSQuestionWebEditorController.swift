...
/// 问题的web编辑器
class TSQuestionWebEditorController: TSWebEditorBaseVC {
...
extension TSQuestionWebEditorController {
    ...
    override func nextProcess() {
        ...
        let complete = {
            ...
            // 发布模型更新
            self.contributeModel?.content = getMarkdown
            self.contributeModel?.content_text = getContent
            // 进入话题选择界面
...
extension TSQuestionWebEditorController {
    ...
    @objc override func leftItemClick() {
        ...
        editorView.getContentMarkdown()
        didGetMarkdownAction = { (data) in
            self.contributeModel?.content = data
        }
        editorView.getContentText()
        didGetContentAction = { (data) in
            self.contributeModel?.content_text = data
        }
