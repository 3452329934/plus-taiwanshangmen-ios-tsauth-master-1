//
//  TSWebEditorBaseVC.swift
//  ThinkSNSPlus
//
//  Created by IMAC on 2020/6/28.
...
class TSWebEditorBaseVC: TSViewController {
    ...
    var imageMap = [Int: TSWebEditorImageNode]()
    /// 是否是草稿箱编辑
    ...
    /// 编辑器 - 编辑视图
    var editorView: TSWebEidtor!
    /// 底部工具栏
    weak var editorToolbar: TSWebEditorToolBar!
    ...
    /// 获取到markdown以及content的值回传
    /// 由于获取markdown或者content的js不能有直接的返回值，所以才需要手动实现，用完即刻销毁
    var didGetMarkdownAction: ((String) -> Void)?
    var didGetContentAction: ((String) -> Void)?
...
extension TSWebEditorBaseVC: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        guard let body = message.body as? Dictionary<String, Any>, let funcName = body["funcName"] as? String, let params = body["params"] as? String else {
            return
        }
//        TSLogCenter.log.debug(body)
        if funcName == "appDebug" {
            jsDebug(params)
        } else if funcName == "appContentFocus" {
            jsContentFocus()
        } else if funcName == "appContentBlur" {
            jsContentBlur()
        } else if funcName == "appContentChange" {
            jsContentChange()
        } else if funcName == "appImageClick", let value = Int(params) {
            jsImageClick(imageIndex: value)
        } else if funcName == "appImageDelete", let value = Int(params) {
            jsImageDelete(imageIndex: value)
        } else if funcName == "appScrollPosition", let value = Double(params) {
            jsScrollPostion(value)
        } else if funcName == "appEnableEditingStyleItems" {
            jsEnableEditingStyleItems(strItems: params)
        } else if funcName == "contentfocus" {
            emojiFocusEditor = false
            editorContentFocus()
        } else if funcName == "getContentText" {
            didGetContentAction?(params)
        } else if funcName == "getContentMarkdown" {
            didGetMarkdownAction?(params)
        }
    }
}
...
extension TSWebEditorBaseVC {
    ...
    /// 加载草稿 非markdown部分
    @objc func loadDraftNoMarkdown() -> Void {
    }
    /// 加载草稿 markdown部分
    ...
    /// 加载数据 非markdown部分
    @objc func loadDataNoMarkdown() -> Void {
    ...
    /// 加载数据 markdown部分
    @objc func loadDataForMarkdownContent() -> Void {
        switch self.editType {
        case .normal:
            break
        case .draft:
    ...
    /// 明确markdown进行加载
    @objc func loadDataWithMarkdown(_ markdown: String) -> Void {
...
extension TSWebEditorBaseVC {
    /// 获取图片文件id
    func getImageIds(complete: @escaping(_ infos: [Int]?) -> Void) {
        // 1. imageMap遍历获取
...
//        }
        // 2. markdown正则判定获取
        didGetMarkdownAction = { [unowned self] (info) in
            self.didGetMarkdownAction = nil
            complete(info.ts_getCustomMarkdownImageId())
        }
...
extension TSWebEditorBaseVC {
    @objc func couldNext(complete: @escaping(_ result: Bool) -> Void) {
        ...
        /// 检测获取的值是否正常
        let cheackAction = {
            guard didGetMarkDown, didGetContent else {
                return
            }
            guard let markdown = markdownString, !markdown.isEmpty, let summary = contentString, !summary.isEmpty else {
                complete(false)
                return
            }
            //除去前后空格
        ...
        /// 实现markdown获取闭包，用完即销毁
        didGetMarkdownAction = { [unowned self] (markdown) in
            self.didGetMarkdownAction = nil
            didGetMarkDown = true
        ...
        /// 实现content获取闭包，用完即销毁
        didGetContentAction = { [unowned self] (content) in
    ...
    /// 子类会重写该方法，已获取到编辑器中内容
    @objc func nextProcess() -> Void {
    }
}

extension TSWebEditorBaseVC {
    /// 保存草稿判断
    @objc func couldSaveDraft(complete: @escaping(_ could: Bool) -> Void) {
        complete(false)
    }
    /// 显示保存草稿提示弹窗
    @objc func showSaveDraftDialogView() -> Void {
    ...
    @objc func rightItemClick() -> Void {
        ...
        couldNext { (result) in
            if result {
                ...
                TSIndicatorWindowTop.showDefaultTime(state: .faild, title: "内容为空")
            }
        }
    }
}
...
extension TSWebEditorBaseVC {
    // 键盘弹出通知处理
    @objc func keyboardWillShowNotificationProcess(_ notification: Notification) -> Void {
        guard let info = notification.userInfo, let duration = info[UIResponder.keyboardAnimationDurationUserInfoKey] as? TimeInterval, let kbEndFrame = info[UIResponder.keyboardFrameEndUserInfoKey] as? CGRect else {
            return
        }
        if emojiFocusEditor {
            return
        ...
        currentKbH = kbHeight
        let animOptions: UIView.AnimationOptions = UIView.AnimationOptions(rawValue: 0)
    ...
    // 键盘隐藏通知处理
    @objc func keyboardWillHideNotificationProcess(_ notification: Notification) -> Void {
        guard let info = notification.userInfo, let duration = info[UIResponder.keyboardAnimationDurationUserInfoKey] as? TimeInterval, let kbEndFrame = info[UIResponder.keyboardFrameEndUserInfoKey] as? CGRect else {
            return
        }
        if emojiFocusEditor {
...
extension TSWebEditorBaseVC {
    ...
    @objc func editorContentFocus() -> Void {
        self.editorToolbar.inputEnable = true
    }
    @objc func editorContentBlur() -> Void {
//        emojiTap = false
...
extension TSWebEditorBaseVC: UIScrollViewDelegate {
    /// 拖动视图时关闭键盘
...
extension TSWebEditorBaseVC: WKNavigationDelegate {
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        let minH: CGFloat = ScreenHeight - TSNavigationBarHeight
        self.editorView.setContentMinHeight(minH)
        self.editorView.setFooterHeight(10)
        // 加载数据
...
extension TSWebEditorBaseVC: TSEditorToolBarProtocol {
    /// 样式点击回调
...
//            }
            if !emojiView.isHidden {
                self.didClickKeyboardBtn(in: editorToolbar)
            }
    ...
    @objc func enableToolBar() {
        self.editorToolbar.inputEnable = true
    }
}

// MARK: - <UIImagePickerControllerDelegate>
extension TSWebEditorBaseVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        guard let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage else {
            picker.dismiss(animated: true, completion: nil)
            return
        }
        let photoFixed = image.fixOrientation()
        self.editorView.focusContentEditor()
        // url: 图片保存到沙盒中的文件
        let imageNode = TSWebEditorImageManager.default.addImage(photoFixed)
        picker.dismiss(animated: true) {
            //self.insertImage(image)
            self.insertImage(photoFixed, url: imageNode.filePath, name: imageNode.name)
            self.editorContentBlur()
        }
    }
...
extension TSWebEditorBaseVC: TSSystemEmojiSelectorViewDelegate {
    ...
    func emojiViewDidSelected(emoji: String) {
        ...
        emojiFocusEditor = true
        self.editorView.blurContentEditor()
        self.perform(#selector(enableToolBar), afterDelay: 0_001)
    }
}
