...
extension GroupLocationController {
    ...
    /// 不显示位置信息
    @objc func noneLocationLabelDidTap() {
...
//        4）如果搜索关键字明显为误输入，则可通过 response.suggestion.keywords法得到搜索关键词建议。
    ...
    func aMapSearchRequest(_ request: Any!, didFailWithError error: Error!) {
        ...
        TSIndicatorWindowTop.showDefaultTime(state: .faild, title: error.debugDescription)
        locationTableView.mj_header.endRefreshing()
        locationTableView.mj_footer.endRefreshing()
    }
}
