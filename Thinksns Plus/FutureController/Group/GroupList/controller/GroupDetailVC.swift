...
class GroupDetailVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func didSelectedTypeBtn(selectedBtn: UIButton) {
    ...
    /// 点击了 退出or转让按钮
    @objc func exitButtonTaped(_ sender: UIButton) {
    ...
    /// 圈内搜索点击了“去发帖“按钮
    @objc func inGroupSearchReleaseButtonTaped(_ noti: Notification) {
        guard let notiGroupId = noti.userInfo?["groupId"] as? Int, notiGroupId == groupId else {
            return
        }
        releaseButtonTaped()
    }
    @objc func notiResReloadGroupInfo(noti: Notification) {
...
extension GroupDetailVC: FeedListViewRefreshDelegate {
    ...
    func feedListTable(_ table: FeedListView, loadMoreDataOf tableIdentifier: String) {
        ...
        GroupNetworkManager.getPosts(groupId: groupId, type: type.rawValue, offset: table.after) { [weak self] (model, message, status) in
            (table as? PostListActionView)?.isRequestList = false
