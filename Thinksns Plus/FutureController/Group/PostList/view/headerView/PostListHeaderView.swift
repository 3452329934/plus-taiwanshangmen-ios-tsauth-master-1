...
extension PostListHeaderView: PostListFixedHeaderViewDelegate {
    ...
    func postListFixedHeaderView(_ view: PostListFixedHeaderView, didSelectedIntroLabelButtonWithNewFrame newFixedViewFrame: CGRect) {
        // 获取弹性视图的 model
        // 更新数据
        let newHeight = newFixedViewFrame.height + TSNavigationBarHeight
        table.contentInset = UIEdgeInsets(top: newHeight, left: table.contentInset.left, bottom: table.contentInset.bottom, right: table.contentInset.right)
        table.contentOffset = CGPoint(x: table.contentOffset.x, y: table.contentOffset.y - (newHeight - originHeight - (originHeight - frame.height)))
        self.stretchModel.headerHeightMin = newFixedViewFrame.height
        load(stretchModel: self.stretchModel)
        // [注意] 这里要手动调用一次刷新界面的方法
        
        updateChildviews(tableOffset: -newHeight)
    }
