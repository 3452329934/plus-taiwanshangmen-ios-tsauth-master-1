...
class StretchTableHeader: UIView {
    ...
    init() {
        ...
        if let image = UIImage(named: "pic_mask"), let cgImage = image.cgImage {
            let maskImage = UIImage(cgImage: cgImage, scale: image.scale, orientation: UIImage.Orientation.downMirrored)
...
extension StretchTableHeader {
    ...
    @objc open func updateChildviews(tableOffset offset: CGFloat) {
