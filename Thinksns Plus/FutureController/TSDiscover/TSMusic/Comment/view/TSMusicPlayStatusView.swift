...
class TSMusicPlayStatusView: UIView {
    ...
    private func layoutAnimationView() {
        ...
        self.AnimationView.setImage(UIImage(named: "IMG_music_ico_suspension_black"), for: UIControl.State.normal)
        self.AnimationView.addTarget(self, action: #selector(pushToPlayVC), for: UIControl.Event.touchUpInside)
    ...
    public func reSetImage(white: Bool) {
        self.AnimationView.setImage(UIImage(named: white ? "IMG_music_ico_suspension_white" : "IMG_music_ico_suspension_black"), for: UIControl.State.normal)
    }

    @objc public func panAction(recognizer: UIPanGestureRecognizer) {
        if recognizer.state == .began {
            self.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.5)
            self.AnimationView.setImage(UIImage(named: "IMG_music_ico_suspension_white"), for: UIControl.State.normal)
        }
