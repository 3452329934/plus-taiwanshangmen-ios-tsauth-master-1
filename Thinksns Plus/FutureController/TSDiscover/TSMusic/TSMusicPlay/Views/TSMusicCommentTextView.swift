//
//  TSMusicCommentTextView.swift
//  ThinkSNSPlus
//
//  Created by IMAC on 2018/11/14.
...
protocol TSMusicCommentTextViewDelegate: NSObjectProtocol {
    func sendTextMessage(message: String, inputBox: AnyObject?)
    func removeToolBar()
    func emojiClick(emoji: String)
}

class TSMusicCommentTextView: UIView, UITextViewDelegate, TSSystemEmojiSelectorViewDelegate {

    typealias ChangeHeightClosure = (CGFloat) -> Void

    private var emojiViewHeight: CGFloat = 0//145
    var safeViewHeight: CGFloat = TSBottomSafeAreaHeight
    ...
    /// true 显示表情或者键盘 false 悬停在底部
    var showOrNot = false

    weak var toolBarViewDelegate: TSMusicCommentTextViewDelegate?
    // MARK -  Lifecycle
    ///
    /// - Parameter frame: 尺寸
    ...
    private func setUI() {
        if showOrNot {
            emojiViewHeight = 145
            safeViewHeight = TSBottomSafeAreaHeight
        } else {
            emojiViewHeight = 0
            safeViewHeight = 0
        }
        ...
        sendButton.snp.makeConstraints { (make) in
            ...
            make.bottom.equalTo(self.snp.bottom).offset(-buttonButtommargin - emojiViewHeight - safeViewHeight)
        ...
        bottomLine.snp.makeConstraints({ (make) in
            ...
            make.bottom.equalTo(self.snp.bottom).offset(-bottomLineBottommargin - emojiViewHeight - safeViewHeight)
            make.height.equalTo(bottomLineHeight)
        })
        ...
        emojiView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(emojiViewHeight + safeViewHeight)
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if selectedRange == nil {
            ...
            attString.addAttributes([NSAttributedString.Key.foregroundColor: textView.textColor, NSAttributedString.Key.font: textView.font], range: NSRange(location: 0, length: attString.length))
            let matchs = TSUtil.findAllTSAt(inputStr: textView.text)
            for item in matchs {
                attString.addAttributes([NSAttributedString.Key.foregroundColor: TSColor.main.theme], range: NSRange(location: item.range.location, length: item.range.length - 1))
            }
    ...
    func setAllComponenSize(textView: UITextView) {
        ...
        let currentComponentHeight = bottomLineBottommargin + bottomLineHeight + newSize.height + emojiViewHeight + safeViewHeight
    ...
    /// 表情和文字输入切换 show true 显示键盘输入框第一响应 false 显示表情
    func changeFrame(show: Bool) {
        if show {
            setInBottom()
        } else {
            emojiViewHeight = 145
            safeViewHeight = TSBottomSafeAreaHeight
            scrollMaxHeight = 120 + emojiViewHeight + safeViewHeight
            let newSize = self.sendTextView.sizeThatFits(CGSize(width: self.sendTextView.frame.size.width, height: CGFloat(MAXFLOAT)))
            let currentComponentHeight = bottomLineBottommargin + bottomLineHeight + newSize.height + emojiViewHeight + safeViewHeight
            ...
            smileButton.snp.updateConstraints { (make) in
                make.left.equalTo(self.snp.left).offset(10)
                make.centerY.equalTo(sendButton.snp.centerY)
                make.size.equalTo(CGSize(width: 25, height: 25))
            }
            ...
            // 设置字数文本框
            showTextNumber.snp.updateConstraints { (make) in
                make.bottom.equalTo(sendButton.snp.bottom)
                make.centerX.equalTo(sendButton.snp.centerX)
            }
    ...
    func setInBottom() {
        ...
        scrollMaxHeight = 120 + emojiViewHeight + safeViewHeight
        let newSize = self.sendTextView.sizeThatFits(CGSize(width: self.sendTextView.frame.size.width, height: CGFloat(MAXFLOAT)))
        let currentComponentHeight = bottomLineBottommargin + bottomLineHeight + newSize.height
        topLine.snp.updateConstraints { (make) in
            make.top.left.right.equalTo(self)
            make.height.equalTo(bottomLineHeight)
        }
        ...
        sendButton.snp.updateConstraints { (make) in
            make.right.equalTo(self.snp.right).offset(-buttonRightmargin)
            make.bottom.equalTo(self.snp.bottom).offset(-buttonButtommargin)
            make.size.equalTo(buttonSize)
        }
        ...
        bottomLine.snp.updateConstraints({ (make) in
            make.left.equalTo(self.snp.left).offset(bottomLineLeftmargin)
            make.right.equalTo(sendButton.snp.left).offset(-bottomLineRightmargin)
            make.bottom.equalTo(self.snp.bottom).offset(-bottomLineBottommargin)
            make.height.equalTo(bottomLineHeight)
        })
