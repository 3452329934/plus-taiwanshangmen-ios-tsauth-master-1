...
//    var currentSong: TSSongObject? = nil
    ...
    override func viewDidLoad() {
        ...
        // 监听耳机插入和拔掉通知
        NotificationCenter.default.addObserver(self, selector: #selector(pauseBecauseRouteChange), name: AVAudioSession.routeChangeNotification, object: nil)
    }
    ...
    func setSliderStyle() {
        ...
        self.musicSliderView.setThumbImage(UIImage(named: "IMG_music_pic_progressbar_circle"), for: UIControl.State.normal)
    }
    ...
    @IBAction func changWropMode(_ sender: UIButton) {
        ...
        case .circulation:
            sender.setImage(UIImage(named: "IMG_music_ico_random"), for: UIControl.State.normal)
            break
        case .random:
            sender.setImage(UIImage(named: "IMG_music_ico_single"), for: UIControl.State.normal)
            break
        case .single:
            sender.setImage(UIImage(named: "IMG_music_ico_inorder"), for: UIControl.State.normal)
            break
        }
    ...
    @objc func pauseBecauseRouteChange(notice: Notification) {
        let data: NSDictionary = notice.userInfo as! NSDictionary
        let routeState = "\(data.object(forKey: AVAudioSessionRouteChangeReasonKey) ?? -1)"
        if routeState == "2" && TSMusicPlayerHelper.sharePlayerHelper.player.status == .Playing {
            self.playOrStop("")
        }
    }
