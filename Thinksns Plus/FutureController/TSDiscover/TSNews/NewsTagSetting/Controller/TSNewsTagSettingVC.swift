...
class TSNewsTagSettingVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func layouBaseUI() {
        ...
        self.closeButton?.setTitle("收起".localized, for: UIControl.State.normal)
        self.closeButton?.addTarget(self, action: #selector(hiddenView), for: UIControl.Event.touchUpInside)
        ...
        self.editableChangeButton?.setTitle("编辑".localized, for: UIControl.State.normal)
        self.editableChangeButton?.addTarget(self, action: #selector(changeEditStatus), for: UIControl.Event.touchUpInside)
        ...
        /// 注册headerView
        self.conllectionView?.register(TSNewsTagSettingVCReusableView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: HeaderIndentifier)
...
// MARK: - UICollenctionViewDelegate
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        let headerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: HeaderIndentifier, for: indexPath) as! TSNewsTagSettingVCReusableView
...
// MARK: - Actions
    ...
    func setEditStyleUI() {
        ...
        if self.eidtEnable {
            ...
            self.editableChangeButton?.setTitle("完成".localized, for: UIControl.State.normal)
        } else {
            self.hintLable?.text = "长按可编辑".localized
            self.editableChangeButton?.setTitle("编辑".localized, for: UIControl.State.normal)
        }
...
// MARK: - private
    ...
    func dragBegin(WithPoint point: CGPoint) {
        ...
        self.conllectionView?.bringSubviewToFront( self.dragingItem!)
