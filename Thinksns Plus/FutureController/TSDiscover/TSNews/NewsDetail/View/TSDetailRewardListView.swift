...
class TSDetailRewardListView: UIView, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    var rewardModel: TSNewsRewardCountModel! {
        didSet {
            ...
            attributeCountString.addAttribute(NSAttributedString.Key.foregroundColor, value: TSColor.small.rewardText, range: nsRange)
            attributeCountString.addAttribute(NSAttributedString.Key.foregroundColor, value: TSColor.small.rewardText, range: nsRange2)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if ((userListDataSource?[indexPath.row].user) != nil) {
