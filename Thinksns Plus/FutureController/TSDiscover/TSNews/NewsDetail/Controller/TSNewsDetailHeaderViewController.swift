...
class TSNewsDetailHeaderViewController: UIViewController, TSDetailRewardListViewDelegate, TSChoosePriceVCDelegate {
    ...
    func load(_ data: NewsDetailModel?) {
        ...
        let rightMargin = ScreenWidth - 15 - self.followControl.minW
        if self.fromLabel.mj_x + self.fromLabel.width > rightMargin {
            let width = self.fromLabel.mj_x + self.fromLabel.width - rightMargin
            self.fromLabel.width -= width
        }
        var from = ""
       ...
       """
        ...
        let getConentHeight = "document.body.offsetHeight;"
        self.markdownView.webView?.evaluateJavaScript(getConentHeight, completionHandler: { (result, error) in
            if let height = result as? CGFloat {
                ...
                self.delegate?.endLoading(headrView: self.view)
    ...
    @objc func pressNewsDetailUnlikeBtn() {
    ...
    @objc func pressNewsDetailLikeBtn() {
