...
// MARK: - UI
    ...
    func updateTags(WithTags tags: [TSNewsTagObject]) {
        ...
        for object in tags {
            ...
            if tag == 0 {
                /// 默认选中第一个按钮 【“推荐”】
                titleButton.setTitleColor(TSNewsTagButtonUX.selectedTitleColor, for: UIControl.State.normal)
...
// MARK: - actions 
    @objc func buttonClicked(button: TSNewsTagButton) {
...
// MARK: - private
    func recordSelectedButton(button: TSNewsTagButton) {
        if self.selectedButton == nil {
            button.setTitleColor(TSNewsTagButtonUX.selectedTitleColor, for: UIControl.State.normal)
            ...
            self.selectedButton?.setTitleColor(TSNewsTagButtonUX.normalTitleColor, for: UIControl.State.normal)
