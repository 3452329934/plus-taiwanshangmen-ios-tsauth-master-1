...
class TSNewsDetailContentTransfrom: TransformType {
    ...
    func transformFromJSON(_ value: Any?) -> Object? {
        if let content = value as? String {
            ...
            return regular.stringByReplacingMatches(in: content, options: [], range:  NSRange(location: 0, length: content.count), withTemplate: "![$1](\(url)/$2)")
        }
        return nil
    }
    ...
    private func convert(_ content: String) -> String {
        ...
        while imgRegex.matches(newContent) == true {
            newContent.replaceAll(matching: imgRegex, with: String(format:"![$1](%@/$2)", url))
        }
        return newContent
    }
}
