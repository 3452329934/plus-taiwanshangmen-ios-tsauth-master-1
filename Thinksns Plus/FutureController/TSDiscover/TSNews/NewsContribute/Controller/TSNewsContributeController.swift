...
class TSNewsContributeController: TSViewController {
    ...
    fileprivate func barckToFront() -> Void {
        if var childVCList = self.navigationController?.children {
...
extension TSNewsContributeController: TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    fileprivate func openCamera() {
        ...
        fileprivate func openLibrary() {
            ...
            guard let imagePickerVC = TZImagePickerController(maxImagesCountTSType: 1, columnNumber: 4, delegate: self, pushPhotoPickerVc: true, square: false, shouldPick: true, topTitle: "设置封面", mainColor: TSColor.main.theme)
                else {
                    return
            }
