...
class CreatTopicVC: TSViewController, TSCustomAcionSheetDelegate, UITextFieldDelegate, UITextViewDelegate, TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, TYAttributedLabelDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(viewBeginEditingNotificationProcess(_:)), name: UITextView.textDidBeginEditingNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(viewEndEditingNotificationProcess(_:)), name: UITextView.textDidEndEditingNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector:#selector(textViewDidChanged(notification:)), name: UITextView.textDidChangeNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector:#selector(textFieldDidChanged(notification:)), name: UITextField.textDidChangeNotification, object: nil)
    }
    ...
    private func initialUI() -> Void {
        ...
        if !isEditPush {
            ...
            self.navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(named: "topbar_back"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(backItemClick))
...
//        self.view.addSubview(mengcengView)
        ...
        titleButton.tag = 666
        titleButton.addTarget(self, action: #selector(titleButtonClick(sender:)), for: UIControl.Event.touchUpInside)
        ...
        introButton.tag = 999
        introButton.addTarget(self, action: #selector(titleButtonClick(sender:)), for: UIControl.Event.touchUpInside)
        ...
        let attribute = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12), NSAttributedString.Key.foregroundColor: UIColor(hex: 0xce2a2a)]
        ...
        if isEditPush {
            ...
            if self.faceImageView.image != nil {
                ...
                if hasChangeFaceImage {
                    guard let imageData = self.faceImageView.image!.jpegData(compressionQuality: 1.0) else {
                        self.nextBtn.isEnabled = true
                        return
                    }
                    // 压缩图片
