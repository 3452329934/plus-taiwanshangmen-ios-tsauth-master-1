...
class TSTopicGroupVCViewController: UIViewController, UIScrollViewDelegate {
    ...
    func setUI() {
        ...
        hotButton.tag = 666
        hotButton.addTarget(self, action: #selector(hotOrNewButtonClick(sender:)), for: UIControl.Event.touchUpInside)
        ...
        newButton.tag = 999
        newButton.addTarget(self, action: #selector(hotOrNewButtonClick(sender:)), for: UIControl.Event.touchUpInside)
    ...
    @objc func hotOrNewButtonClick(sender: UIButton) {
