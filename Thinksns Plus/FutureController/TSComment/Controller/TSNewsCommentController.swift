...
class TSNewsCommentController: TSCommentListController {
    ...
    override func requestData(_ type: TSListDataLoadType) -> Void {
        ...
        case .refresh:
            ...
            TSNewsTaskManager().refreshNewsData(newsID: self.newsId, limit: self.limit, complete: { [weak self](newsFullModel, _, code) in
                ...
                self?.mjFootViewHiddenState = newsFullModel.comment.count < self!.limit
                // 加载markdown视图
    ...
    // MARK: - private
    fileprivate  func tableScrollowAnimation(_ offset: CGFloat) {
        let minY: CGFloat = 0
    ...
    @objc func updataFollowStatus(notice: NSNotification) {
...
extension TSNewsCommentController {
    ...
    func creatTableView() {
        ...
        tableView.frame = CGRect(x: 0, y: TSStatusBarHeight, width: ScreenSize.ScreenWidth, height: ScreenSize.ScreenHeight - TSStatusBarHeight)
        ...
        tableView.contentInset = UIEdgeInsets(top: TSNavigationBarHeight - TSStatusBarHeight, left: 0, bottom: 0, right: 0)
...
extension TSNewsCommentController: DetailHeaderViewControllerDelegate {
    /// 先显示文本内容，给一个预估的高度，然后等待图片显示之后更新高度
    func endLoading(headrView view: UIView?) {
        self.endLoading()
...
extension TSNewsCommentController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:     // 相关资讯
            ...
            if cell == nil {
                cell = TSNewsListCell(style: UITableViewCell.CellStyle.default, reuseIdentifier: identifier)
            }
...
extension TSNewsCommentController {
    ...
    override func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        case 1:     // 广告
            sectionHeight = CommentCountsViewUX.top
        case 2:     // 评论
            sectionHeight = TSCommentHeaderView.headerHeight
        default:
            break
        }
        if sectionHeight > maxHeaderHeight {
            maxHeaderHeight = sectionHeight
        }
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        var yPoint = self.commentToolBar!.frame.height
        if UIApplication.shared.statusBarFrame.size.height != TSStatusBarHeight {
            yPoint += TSStatusBarHeight
        }
        let navTopY = -self.navigationBar.frame.height + TSStatusBarHeight + 1
        let navseparateY = -self.navigationBar.frame.height / 2
        let navBottomY: CGFloat = 0
        let shouldHidden = self.navigationBar.frame.minY < navseparateY
        let shouldAnimation = ( self.navigationBar.frame.minY < navBottomY && self.navigationBar.frame.minY > navTopY)
        if !shouldAnimation {
            return
        }
        UIView.animate(withDuration: 0.2, animations: {
            if shouldHidden {
                ...
                self.commentToolBar!.frame = CGRect(x: 0, y: UIScreen.main.bounds.height - yPoint, width: self.commentToolBar!.frame.width, height: self.commentToolBar!.frame.height)
            }
//            self.tableView.frame = CGRect(x: 0, y: self.navigationBar.frame.maxY, width: self.tableView.frame.width, height: (self.commentToolBar!.frame.minY - self.navigationBar.frame.maxY))
        })
    }
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //固定header
        let sectionHeaderHeight: CGFloat = maxHeaderHeight
        if scrollView.contentOffset.y <= 0 {
            scrollView.contentInset = UIEdgeInsets(top: TSNavigationBarHeight - TSStatusBarHeight, left: 0, bottom: 0, right: 0)
        ...
        if scrollView.contentOffset.y > (self.navigationBar.frame.height + self.commentToolBar!.frame.height) {
...
//            self.tableScrollowAnimation(direction)
            if scrollView.contentOffset.y <= 0 {
                self.navigationBar.frame = CGRect(x: 0, y: 0, width:   self.navigationBar.frame.width, height: self.navigationBar.frame.height)
                self.commentToolBar!.frame = CGRect(x: 0, y: UIScreen.main.bounds.height - yPoint, width: self.commentToolBar!.frame.width, height: self.commentToolBar!.frame.height)
//                self.tableView.frame = CGRect(x: 0, y: self.navigationBar.frame.maxY, width: self.tableView.frame.width, height: (self.commentToolBar!.frame.minY - self.navigationBar.frame.maxY))
            }
        }
        self.tableView.mj_footer.isHidden = self.mjFootViewHiddenState
    }
