...
//        }
    ...
    private func setupShortContentLabel(with content: String, model: TSQuestionDetailModel) -> Void {
        ...
        attContent.addAttribute(NSAttributedString.Key.paragraphStyle, value: paragraphStyle, range: NSRange(location: 0, length: attContent.length))
        ...
        let truncationToken = NSAttributedString.attachmentString(withContent: seeMoreLabel, contentMode: UIView.ContentMode.center, attachmentSize: seeMoreLabel.size, alignTo: seeMoreLabel.font!, alignment: YYTextVerticalAlignment.center)
