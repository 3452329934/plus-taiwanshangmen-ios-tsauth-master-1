...
//        previewView.addSubview(loadingView)
    ...
    func setupRecorder() {
        ...
        recorder.maxRecordDuration = CMTimeMakeWithSeconds(10, preferredTimescale: 600)
        /// maxTime, maxTime * fps
        recorder.maxRecordDuration = CMTimeMakeWithSeconds(60, preferredTimescale: 60 * 30)
...
//        audioConfig.format = Int32(kAudioFormatMPEG4AAC)
        ...
        focusView.autoresizingMask = [UIView.AutoresizingMask.flexibleBottomMargin, UIView.AutoresizingMask.flexibleHeight, UIView.AutoresizingMask.flexibleLeftMargin, UIView.AutoresizingMask.flexibleRightMargin, UIView.AutoresizingMask.flexibleTopMargin, UIView.AutoresizingMask.flexibleWidth]
    ...
    // MARK: set btn action
    @objc func action(reverseCamera btn: UIButton) {
        recorder.switchCaptureDevices()
    }

    @objc func action(whiteBtn btn: UIButton) {
    ...
    @objc func action(finishRecorderBtn btn: UIButton) {
        recorder.pause {
    ...
    @objc func action(undoRecorderBtn btn: UIButton) {
    ...
    @objc func action(recorderbtn recorderBtn: UIButton) {
        ...
        case .autoRecording: // 按钮自动录制的情况下,被点击了一下,进入正常模式
            ...
            recorder.pause()
            VideoRecorderExportHelper.exportVideo(recordSession: self.recorder.session, minDuration: minDuration, completionHandler: nil)
        case .manuRecording:
    ...
    @objc func longPress(_ longPress: UILongPressGestureRecognizer) {
        ...
        if longPress.state == UIGestureRecognizer.State.began && type == .normal {
