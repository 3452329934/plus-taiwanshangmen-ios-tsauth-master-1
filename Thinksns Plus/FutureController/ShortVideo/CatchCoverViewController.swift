...
class CatchCoverViewController: UIViewController {
    ...
    func catchCoverImage(segmentsIndex: Int, time: CMTime) {
        ...
        assetImage.requestedTimeToleranceAfter = CMTime.zero
        assetImage.requestedTimeToleranceBefore = CMTime.zero
    ...
    @IBAction func slidervalueChanged(_ sender: UISlider) {
        ...
        let time = CMTimeMakeWithSeconds(groupIndeValue, preferredTimescale: Int32(1 * USEC_PER_SEC))
    ...
    @IBAction func fininshBtnAction(_ sender: Any) {
        ...
        VideoRecorderExportHelper.exportVideo(recordSession: recordSession) { [weak self] exportSession in
            ...
            if let error = exportSession.error {
                ...
                VideoRecorderExportHelper.saveVideoToPhotosAlbum(videoURL: exportSession.outputUrl!, completion: { [weak self] path, error in
                    guard let `self` = self else {
                        return
                    }
                    if let error = error {
