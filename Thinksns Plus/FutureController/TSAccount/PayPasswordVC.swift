...
//        pwdTF.addTarget(self, action: #selector(textFieldDidChange(tf:)), for: .editingChanged) 
    ...
    func registerNoti() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(textFieldValueDidChanged(notification:)), name: UITextField.textDidChangeNotification, object: nil)
    }
    /// 处理确定按钮状态以及提示语
    ...
    /// 输入框通知
    @objc func textFieldValueDidChanged(notification: Notification) {
