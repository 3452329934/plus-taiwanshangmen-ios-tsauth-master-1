...
class TSMomentDetailVC: TSViewController, TSMomentDetailNavViewDelegate/* 导航视图 代理 */, TSMomentDetailHeaderViewDelegate/* headerView 代理 */, UITableViewDelegate, TSMomentDetailToolbarDelegate/* 工具栏点击代理事件 */, TSKeyboardToolbarDelegate, TSCustomAcionSheetDelegate/* 弹出视图的点击代理 */, TSChoosePriceVCDelegate, ZFPlayerDelegate {
    ...
    var playerControlView: CustomPlayerControlView?
    // 离开页面时是否正在播放视频
    ...
    func playWith(model: TSMomentListCellModel) {
        ...
        playerControlView = CustomPlayerControlView()
        playerControlView?.zf_playerPlayEndBlock = { [weak self] in
            self!.headerView?.playBtn.isHidden = true
        }
        playerControlView!.showFullScreenPlayEndView = true
        playerControlView?.fullScreenPlayEndView.imageColor = .white
        if let shareView = self.configShareView() {
            playerControlView!.setFullScreenShareItemsModel(shareView.getVideoShareItems()) { (model) in
                shareView.shareByShareName(model.tag, fatherViewTag: 0, finishBlock: { (state) in
                    //分享完成
                })
            }
        }
        playerView?.playerControlView(playerControlView!, playerModel: playerModel!)
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        let playState = !(self.playerView?.isPauseByUser ?? true)
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if self.navigationController?.viewControllers.count == 3, let playerView = self.playerView, playerView.isPauseByUser == false {
            self.isPlaying = true
...
//        })
    ...
    func configShareView() -> ShareListView? {
        ...
        shareView.config(URLString: ShareURL.feed.rawValue + "\(model.data!.feedIdentity)", image: image, description: description, title: title)
        return shareView
    }
    // MARK: - Notification
    ...
    @objc func appWillResignActive() {
        ...
        if playerView?.isPauseByUser == false {
            playerView?.pause()
        }
    }
...
extension TSMomentDetailVC: ShareListViewDelegate {
    ...
    func didClickMessageButon(_ shareView: ShareListView, fatherViewTag: Int, feedIndex: IndexPath?, model: TSmessagePopModel) {
        self.playerControlView?.setPlayerFullScreen(false)
